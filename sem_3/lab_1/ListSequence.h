...
void ListSequence<T>::Remove(T item) {
	if (this->isEmpty)
		...
		for (int j = 0; j < this->length; j++) {
			if (ptr->data == item && ptr->prev != nullptr && ptr->next != nullptr) {
				this->length--;
			...
			else if (ptr->data == item && ptr->prev == nullptr) {
				this->length--;
				this->first = ptr->next;
				ptr->next->prev = nullptr;
				Node<T> *ptr1 = this->first;
				delete ptr;
				ptr = ptr1;
				j--;
			}
			else if (ptr->data == item && ptr->next == nullptr) {
				this->length--;
				this->last = ptr->prev;
				ptr->prev->next = nullptr;
				Node<T> *ptr1 = this->last;
				delete ptr;
				ptr = ptr1;
				j--;
			}
...
void ListSequence<T>::printAll() {
	if (this->isEmpty)
		...
		for (i = 0; ptr != this->last; i++) {
			cout << "Ёлемент " << i << " : " << ptr->data << endl;
			ptr = ptr->next;
		}
		cout << "Ёлемент " << i << " : " << this->last->data << endl << endl;
	}
}
