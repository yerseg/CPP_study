...
void Testing() {
	...
	if (flag1 == flag2 == flag3 == TRUE)
		printf("\nТЕСТИРОВАНИЕ завершено: все функции работают КОРРЕКТНО\n\n");
	else {
		if (flag1 != TRUE)
			printf("\nТЕСТИРОВАНИЕ завершено: ОШИБКА в конкатенации\n");
		if (flag2 != TRUE)
			printf("\nТЕСТИРОВАНИЕ завершено: ОШИБКА в извлечении\n");
		if (flag3 != TRUE)
			printf("\nТЕСТИРОВАНИЕ завершено: ОШИБКА в поиске\n");
		else
			;
	}
}
